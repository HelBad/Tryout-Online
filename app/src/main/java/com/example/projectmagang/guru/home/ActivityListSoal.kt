...
import com.example.projectmagang.adapter.ListSoalAdapter
import com.example.projectmagang.api.UtilsAPI
import com.example.projectmagang.model.DataSoal
import com.example.projectmagang.model.ResponseListDataSoal
...
class ActivityListSoal : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        getSoal(intent.getIntExtra("id_mapel",0))
        ...
        btn_tambahSoal.setOnClickListener {
            ...
            test.putExtra("id_mapel",intent.getIntExtra("id_mapel",0))
            startActivity(test)
        }
    }
    ...
    fun getSoal(id : Int){
        UtilsAPI().apiService.mapelSoal(id).enqueue(object : Callback<ResponseListDataSoal> {
            override fun onFailure(call: Call<ResponseListDataSoal>, t: Throwable) {
                t.printStackTrace()
                showMessage("Server Error")
                loading.visibility = View.GONE
            }
            override fun onResponse(call: Call<ResponseListDataSoal>, response: Response<ResponseListDataSoal>) {
                if(response.isSuccessful) {
                    ...
                    else { kosongSoal.visibility = View.VISIBLE }
                }
            }
        })
    }
    ...
    fun hapusSoal(id : Int, position : Int, dataSoal : DataSoal) {
        ...
        dialog.setPositiveButton("Hapus") { dialog, which ->
            UtilsAPI().apiService.deleteSoal(id).enqueue(object : Callback<ResponseMessage> {
        ...
        dialog.setNegativeButton("Batal") { dialog, which -> dialog.dismiss() }
        dialog.show()
    }
