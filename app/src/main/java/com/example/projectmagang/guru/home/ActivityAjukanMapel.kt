...
import com.example.projectmagang.adapter.MapelTersediaAdapter
import com.example.projectmagang.api.UtilsAPI
import com.example.projectmagang.model.DataMapelDiajukan
import com.example.projectmagang.model.ResponseListDataMapelDiajukan
...
class ActivityAjukanMapel : AppCompatActivity() {
        ...
        rvMapelTersedia.apply {
            ...
            mapelTersediaAdapter.setOnTambahMapelCallback(object : MapelTersediaAdapter.OnItemClickCallback {
                override fun onItemClicked(data: DataMapelDiajukan, position: Int) {
                    tambahMapel(data,SP.getString("id_user","").toString(), position)
                }
            })
    ...
    fun tambahMapel(data : DataMapelDiajukan, id : String , position : Int) {
        ...
        dialog.setPositiveButton("Iya"){dialog, which ->
            UtilsAPI().apiService.tambahMapelDiajukan(data.id, id).enqueue(object : Callback<ResponseMessage> {
    ...
    fun getMapelTersedia(){
        UtilsAPI().apiService.mapelTersedia().enqueue(object : Callback<ResponseListDataMapelDiajukan>{
